...
function deleteOne(str, isTrue) {
  return isTrue ? str.slice(1) : str.slice(0, -1);
}
...
console.log(isThisAnEmail("ombretta@gmail.com"));
console.log(isThisAnEmail("non è una mail"));
...
console.log(howManyDays("2000-04-11"));
...
function isTodayMyBirthday() {
  const today = new Date();
  const birthday = new Date(today.getFullYear(), 3, 11);
...
const movie = { Title: "The Lord of the Rings: The Fellowship of the Ring", Year: "2001", imdbID: "tt0120737" };
console.log(deleteProp(movie, "imdbID"));
...
function newestMovie(movies) {
  return movies.reduce((newest, movie) => (parseInt(movie.Year) > parseInt(newest.Year) ? movie : newest), movies[0]);
}
...
// Crea una copia dell'array 'movies' per evitare modifiche dirette all'array originale

function removeByTitle(title) {
  const newMovies = movies.filter(movie => movie.Title !== title);
  return newMovies;
}
console.log(removeByTitle('The Lord of the Rings: The Fellowship of the Ring'));
...
*/
/*function addToList(newItem) {const list = document.getElementById('myList');
...
*/
function clearList() {const list = document.getElementById('myList');
  innerHTML = '';}
  clearList();
...
function addClassToRows() {
  ...
  rows.forEach(row => {classList.add('test');
  });
}
addClassToRows();
// [EXTRA] JS Avanzato
